...
const typeNum = (arr) => {
  ...
  let numberss = arr.filter(numbersOnly);
  return numberss;
};
console.log(typeNum ([3,4,"5"]))
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const notInFirstArray = (biddenValues, arr) => {
  // Solution code here...
  let temporaryArray = arr.filter(function (item) {
    return !biddenValues.includes(item);
  });
  biddenValues = biddenValues.filter(function (item) {
